    ...
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            ...
            $db->sql("
                    ...
                    FROM tbl_questions q 
                    JOIN " . $table . " c ON q.category_id = c.id 
                    WHERE q.category_id = $category_id AND c.type = $type AND c.tag = '$tag'
                ");
            respond($db->getResult());
            ...
            $totalQuery = "
                    ...
                    FROM tbl_questions q 
                    JOIN " . $table . " c ON q.category_id = c.id 
                    WHERE (q.question LIKE '%$search%' OR q.optiona LIKE '%$search%' OR q.optionb LIKE '%$search%' OR q.optionc LIKE '%$search%' OR q.optiond LIKE '%$search%') AND c.id LIKE '%$category%' AND q.question LIKE '%$search%' AND c.type = $type AND c.tag = '$tag'
                ";
            $db->sql($totalQuery);
            ...
            $query = "
                    ...
                    FROM tbl_questions q 
                    JOIN " . $table . " c ON q.category_id = c.id 
                    WHERE (q.question LIKE '%$search%' OR q.optiona LIKE '%$search%' OR q.optionb LIKE '%$search%' OR q.optionc LIKE '%$search%' OR q.optiond LIKE '%$search%') AND c.id LIKE '%$category%' AND c.type = $type AND c.tag = '$tag'
            ...
            $db->sql("
                    ...
                    FROM tbl_questions q 
                    JOIN " . $table . " c ON q.category_id = c.id 
                    WHERE c.type = $type AND c.tag = '$tag'
                ");
            respond($db->getResult());
        }
        break;
