...
function handleGetRequest($db, &$response, $baseURL)
{
    ...
    if (isset($_GET['id'])) {
        ...
        $db->select('tbl_categories', 'id, category_name, pdf, type', null, $whereClause);
        $result = $db->getResult();
        ...
        $query = "SELECT id, category_name, pdf, type FROM tbl_categories WHERE $whereClause LIMIT $limit OFFSET $offset";
        $db->sql($query);
        $data = $db->getResult();
        foreach ($data as &$item) {
            $item['pdf'] = $item['pdf'] != null ? $baseURL . $item['pdf'] : 'Not Set';
