...
if (isset($_POST['question_id']) && isset($_POST['update_question'])) {
    ...
    $question = $db->escapeString($_POST['edit_question']);
    $category = $db->escapeString($_POST['edit_category']);
    $subcategory = (isset($_POST['edit_subcategory'])) ? $db->escapeString($_POST['edit_subcategory']) : 0;

    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['update_language_id']) : 0;

    $question_type = $db->escapeString($_POST['edit_question_type']);

    $a = $db->escapeString($_POST['edit_a']);
    $b = $db->escapeString($_POST['edit_b']);
    $c = ($question_type == 1) ? $db->escapeString($_POST['edit_c']) : "";
    $d = ($question_type == 1) ? $db->escapeString($_POST['edit_d']) : "";
    if ($fn->is_option_e_mode_enabled()) {
        $e = ($question_type == 1) ? $db->escapeString($_POST['edit_e']) : "";
    }
    $level = $db->escapeString($_POST['edit_level']);
    $answer = $db->escapeString($_POST['edit_answer']);
    $note = $db->escapeString($_POST['edit_note']);
...
// 54. add_dictionary
if (isset($_POST['title']) && isset($_POST['add_dictionary'])) {
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;
    $title = $db->escapeString($_POST['title']);
    $detail = "0";

    $sql = "INSERT INTO `tbl_dictionary` (`language_id`, `title`) VALUES ('" . $language_id . "','" . $title . "')";
    $db->sql($sql);
    $res = $db->getResult();
    echo '<label class="alert alert-success">Learning created successfully!</label>';
}
...
if (isset($_POST['question']) && isset($_POST['add_learning_question'])) {
    ...
    $sql = "INSERT INTO `tbl_learning_question`(`learning_id`, `question`, `question_type`, `optiona`, `optionb`, `optionc`, `optiond`, `optione`, `answer`,`note`) VALUES 
	('" . $learning_id . "','" . $question . "','" . $question_type . "','" . $a . "','" . $b . "','" . $c . "','" . $d . "','" . $e . "','" . $answer . "','" . $note . "')";
...
if (isset($_POST['question']) && isset($_POST['add_exam_question'])) {
    $question = $db->escapeString($_POST['question']);
    $note = $db->escapeString($_POST['note']);
    $learning_id = $db->escapeString($_POST['learning_id']);
    ...
    $sql = "INSERT INTO `tbl_exam_question`(`learning_id`, `question`, `question_type`, `optiona`, `optionb`, `optionc`, `optiond`, `optione`, `answer`,`note`) VALUES 
	('" . $learning_id . "','" . $question . "','" . $question_type . "','" . $a . "','" . $b . "','" . $c . "','" . $d . "','" . $e . "','" . $answer . "','" . $note . "')";
...
// 50. add_dictionary_data
if (isset($_POST['word']) && isset($_POST['add_dictionary_data'])) {
    $question = $db->escapeString($_POST['word']);
    $note = $db->escapeString($_POST['meaning']);
    $learning_id = $db->escapeString($_POST['learning_id']);


    $sql = "INSERT INTO `tbl_dictionary_data`(`dictionary_id`, `word`, `meaning`) VALUES ('" . $learning_id . "','" . $question . "','" . $note . "')";

    $db->sql($sql);
    $res = $db->getResult();
    echo '<label class="alert alert-success">Word created successfully!</label>';
}


// 51. update_dictionary_word
if (isset($_POST['meaning_id']) && isset($_POST['update_dictionary_word'])) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
...
// 52. delete_learning_question
if (isset($_GET['delete-exam']) && $_GET['delete-exam'] != '') {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    ...
    $sql = 'DELETE FROM `tbl_exam_question` WHERE `id`=' . $id;
    if ($db->sql($sql)) {
        echo 1;
    } else {
        echo 0;
...
if (isset($_POST['import_dictionary_data']) && $_POST['import_dictionary_data'] == 1) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $count = $count1 = 0;
    $filename = $_FILES["dictionary_data_file"]["tmp_name"];
    $file_extension = pathinfo($_FILES["dictionary_data_file"]["name"], PATHINFO_EXTENSION);
    if ($_FILES["dictionary_data_file"]["size"] > 0 && $file_extension == "csv") {
        $file = fopen($filename, "r");
        ...
        if ($empty_value_found == TRUE) {
            ...
            while (($emapData1 = fgetcsv($file, 10000, ",")) !== FALSE) {
                if (count($emapData1) > 2) {
                    $emapData1[0] = $db->escapeString($emapData1[0]);
                    $emapData1[1] = $db->escapeString($emapData1[1]);
                    $emapData1[2] = $db->escapeString($emapData1[2]);
                    $count1++;
                    if ($count1 > 1) {
                        if (count($emapData1) > 2) {
                            $sql = "INSERT INTO `tbl_dictionary_data`(`dictionary_id`, `word`, `meaning`) VALUES 
						('$emapData1[0]','$emapData1[1]','$emapData1[2]')";
                            $db->sql($sql);
                        }
                    }
                }
...
if (isset($_POST['tbl_learning_question']) && $_POST['tbl_learning_question'] == 1) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $count = $count1 = 0;
    $filename = $_FILES["tbl_learning_question"]["tmp_name"];
    $file_extension = pathinfo($_FILES["tbl_learning_question"]["name"], PATHINFO_EXTENSION);
    if ($_FILES["tbl_learning_question"]["size"] > 0 && $file_extension == "csv") {
        $file = fopen($filename, "r");

        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if (count($emapData) > 2) {
                $emapData[0] = $db->escapeString($emapData[0]); //learning_id
                $emapData[1] = $db->escapeString($emapData[1]);     //question
                $emapData[2] = $db->escapeString(trim($emapData[2]));   //question_type
                $emapData[3] = $db->escapeString($emapData[3]);    // optiona
                $emapData[4] = $db->escapeString($emapData[4]);    // optionb
                $emapData[5] = $db->escapeString($emapData[5]);    // optionc
                $emapData[6] = $db->escapeString($emapData[6]);    // optiond
...
//tbl_learning_detail
if (isset($_POST['tbl_learning_detail']) && $_POST['tbl_learning_detail'] == 1) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $count = $count1 = 0;
    $filename = $_FILES["tbl_learning_detail"]["tmp_name"];
    $file_extension = pathinfo($_FILES["tbl_learning_detail"]["name"], PATHINFO_EXTENSION);
    if ($_FILES["tbl_learning_detail"]["size"] > 0 && $file_extension == "csv") {
        $file = fopen($filename, "r");
            ...
            while (($emapData1 = fgetcsv($file, 10000, ",")) !== FALSE) {
                if (count($emapData1) > 2) {
                    ...
                    if ($count1 > 1) {
                        if (count($emapData1) > 2) {
                            $sql = "INSERT INTO `tbl_learning_detail`(`learning_id`, `detail`, `image`, `headline`, `headline_meaning`) VALUES 
						('$emapData1[0]','$emapData1[1]','$emapData1[2]','$emapData1[3]','$emapData1[4]')";
                            $db->sql($sql);
                        }
                    }
                }
...
//tbl_exam_question
if (isset($_POST['tbl_exam_question']) && $_POST['tbl_exam_question'] == 1) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $count = $count1 = 0;
    $filename = $_FILES["tbl_exam_question"]["tmp_name"];
    $file_extension = pathinfo($_FILES["tbl_exam_question"]["name"], PATHINFO_EXTENSION);
    if ($_FILES["tbl_exam_question"]["size"] > 0 && $file_extension == "csv") {
        $file = fopen($filename, "r");
