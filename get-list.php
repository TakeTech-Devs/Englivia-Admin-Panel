...
// 17. learning_zone
if (isset($_GET['table']) && $_GET['table'] == 'dictionary_zone') {
    $offset = 0;
    $limit = 10;
    $sort = 'q.id';
    $order = 'DESC';
    ...
    $sql = "SELECT COUNT(q.id) as total FROM `tbl_dictionary` q " . $left_join . " " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT q.*, l.language FROM `tbl_dictionary` q " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
...
if (isset($_GET['table']) && $_GET['table'] == 'learnning_detail') {
    ...
    $sql = "SELECT COUNT(q.id) as total FROM `tbl_learning_detail` q " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
...
// 18. learnning_question
if (isset($_GET['table']) && $_GET['table'] == 'dictionary_data') {
    $offset = 0;
    $limit = 10;
    $sort = 'q.id';
    $order = 'DESC';
    ...
    $sql = "SELECT COUNT(q.id) as total FROM `tbl_dictionary_data` q " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT q.* FROM `tbl_dictionary_data` q " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
...
if (isset($_GET['table']) && $_GET['table'] == 'exam_question') {
    $offset = 0;
    $limit = 10;
    $sort = 'q.id';
    $order = 'DESC';
    ...
    $sql = "SELECT COUNT(q.id) as total FROM `tbl_exam_question` q " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT q.* FROM `tbl_exam_question` q " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    ...
    foreach ($res as $row) {
        $operate = "<a class='btn btn-xs btn-primary edit-question' data-id='" . $row['id'] . "' data-toggle='modal' data-target='#editQuestionModal' title='Edit'><i class='fas fa-edit'></i></a>";
        $operate .= "<a class='btn btn-xs btn-danger delete-exam' data-id='" . $row['id'] . "' title='Delete'><i class='fas fa-trash'></i></a>";
